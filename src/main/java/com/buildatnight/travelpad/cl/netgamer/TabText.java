package com.buildatnight.travelpad.cl.netgamer;
...
public class TabText {
     ...
     * @param maxLength desired string length (in px)
     ...
     */
    private Object[] pxSubstring(String str, int maxLength, boolean mono) {
        ...
        for (char ch : str.toCharArray()) {
            pxCount += pxLen(ch, mono);
            //if(length of str is longer than allowed, break)
            if (pxCount > maxLength) break;
            ++literalCharCount;
            totalPx = pxCount;
        }
        Travelpad.log("pxSubstring - str:" + str + " limit:" + maxLength + " returning:" + str.substring(0, literalCharCount) + " pxLength:" + totalPx);
        return new Object[]{str.substring(0, literalCharCount), totalPx};
    }
